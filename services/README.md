...
1. The service must:
  ...
  - Follow the behaviors for version negotiation:
    - Return latest version with 200 OK if no version specified.
    ...
    - Set Sunset header (RFC 7231 HTTP-date) if removal is scheduled.
...
2. The service must be modular and split across multiple files according to best practices for the specified language.
  - Separate routing, controllers/handlers, models/entities, services/business logic, and version/deprecation/sunset/removal negotiation.
  - Possible to easily support more than once version at once.
